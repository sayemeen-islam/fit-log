      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8'>
        {
          data.map((workout:IWorkout)=>{
            ...
            return <Link key={workout.id} href={`/workouts/${workout.id}`} className='hover:border border-[#9CA3AF] rounded-2xl'><WorkoutCard key={workout.id} workout={workout}></WorkoutCard></Link>
          })
        }
      </div>
    </section>
